...
public final class Cas
{
    ...
    private static final String LOGIN                   =
                                                            "https://auth.vt.edu/login?service=https://webapps.banner.vt.edu/banner-cas-prod/authorized/banner/SelfService";
    /**
     * The URL to logout of CAS entirely.
     */
    private static final String CAS_LOGOUT              =
                                                            "https://auth.vt.edu/logout";
    /**
     * String to check for recovery options screen with.
     */
    private static final String RECOVERY_OPTIONS_STRING =
                                                            "You have not updated account recovery options in the past";
    ...
    private File                cert;
    /**
     * The path on which to save the SSL certificate. May be null, causes
     * default path to be used.
     */
    private String              certFilePath;
    /**
     * Boolean value denoting whether this CAS object has an active connection
     * to the VT CAS system. True if its active, false otherwise.
     */
    ...
    /**
     * Indicates if the username and password values stored in this Cas object
     * are valid for initiating a Cas session. Note: it is false when username
     ...
     * @param password
     *            a character array that is a HokieSpa password.
     * @throws WrongLoginException
     *             indicates that the username or password was incorrect.
     */
    public Cas(char[] username, char[] password)
        throws WrongLoginException
    {
     ...
     *            a character array that is a HokieSpa password.
     * @param filePath
     *            the path to the file to save the SSL certificate in.
     * @throws WrongLoginException
     *             indicates that the username or password was incorrect.
     */
    public Cas(char[] username, char[] password, String filePath)
        throws WrongLoginException
    {
     ...
     *            the new password to use.
     * @param filePath
     *            a new filePath to use for the SSL certificate.
     * @throws WrongLoginException
     *             indicates that the username or password was incorrect.
     */
    ...
    private boolean grabCertificate()
    {
        ...
        try
        {
            ...
            if (certs.length > 0)
            {
                ...
                cert = new File("assets/auth.vt.edu.jks");
...
// future
    ...
    /**
     * Takes in a username and password and logs into CAS with the supplied
     * username and password. Also takes in a String that specifies the
     * directory where the SSL should be saved. This is especially useful in
     ...
     *            the password of the user.
     * @param filePath
     *            the file path that the SSL certificate should be saved in.
     * @return returns true if successful, false if there was an IOException,
     *         and false if the Certificate wasn't correctly obtained.
     * @throws WrongLoginException
     *             indicates that the username or password was incorrect
...
// future
                ...
                // check to see if the login was successful, that is, had a
// correct PASSWORD and USERNAME
                ...
                else if (checkRecovery != null
                    && checkRecovery.text().contains(RECOVERY_OPTIONS_STRING))
                {
